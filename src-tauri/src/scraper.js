(function () {
    ...
    function analyze() {
        // Browser-side analysis: colors + fonts + metadata only
        // Images and text are handled server-side (like webscrap.py)
        const data = {
            colors: [],
            fonts: [],
            metadata: {
        ...
        // Emit result to Rust backend
        if (window.__TAURI__ && window.__TAURI__.core) {
            window.__TAURI__.core.invoke('complete_analysis', { data })
                .then(() => console.log('Browser analysis sent'))
